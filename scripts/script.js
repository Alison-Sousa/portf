...
// Reveal for Portfolio section (same style as Contact)
sr.reveal("#portfolio", {
  duration: 1000,
  distance: "50px",
  easing: "cubic-bezier(0.5, -0.01, 0, 1.005)",
...
function showButtons() {
  ...
  if (
    ...
    const openMenuTwoEl = document.getElementById("openMenuTwo");
    if (openMenuTwoEl) {
      if (window.innerWidth <= 1000) {
        openMenuTwoEl.style.display = "block";
      } else {
        openMenuTwoEl.style.display = "none";
      }
    }
...
const openMenuTwoBtn = document.getElementById("openMenuTwo");
if (openMenuTwoBtn) {
  openMenuTwoBtn.addEventListener("click", () => {
    openMenu(true);
  });
}
...
// --- BOTÃO VOLTAR AO TOPO DIRETO ---
window.addEventListener("DOMContentLoaded", function() {
  var btn = document.getElementById("backToTop");
  if (!btn) return;
  // Mostrar/ocultar botão ao rolar
  window.addEventListener("scroll", function() {
    if (window.scrollY > 100 || document.body.scrollTop > 100 || document.documentElement.scrollTop > 100) {
      btn.style.display = "flex";
    } else {
      btn.style.display = "none";
    }
  });
  // Clique: scroll instantâneo e suave
  ...
  // Touch: scroll instantâneo e suave
